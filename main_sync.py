    ...
    if IPaddress != "127.0.0.1":
        ...
        if(len(l) > 0 or len(l_in_out) > 0):
            ...
            if response.status_code == 200:
                ...
                #Redirect to login page due to token expire
            l.clear()
            l_in_out.clear()
            conn.commit()
...
if __name__ == "__main__":
    ...
    c.execute('''SELECT DISTINCT DATE FROM attendance WHERE SYNCED = 'False' AND DATE != ?  ''' , (d, ))
    temp_ls = c.fetchall()
    for row in temp_ls:
        working_date = row[0]
        ls = []
        c.execute(''' SELECT DATE, details.C_ID, FNAME, LNAME, ATTEND
                    FROM attendance 
                    INNER JOIN details ON attendance.C_ID = details.C_ID 
                    WHERE SYNCED = 'False' AND DATE != ?  ''' , (d, ))
        for row in c.fetchall():
    ...
    c.execute('''SELECT C_ID, DATE_OUT, TIME_OUT, DATE_IN, TIME_IN FROM in_and_out WHERE SYNCED = 'False' AND TIME_IN IS NOT NULL AND DATE_IN != ?''', (d, ))
    l_in_out = []
    for row in c.fetchall():
        obj = {
            "child_id": row[0],
